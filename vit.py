...
class ViT(nn.Module):
    def __init__(self, img_size=128, patch_size=16, emb_dim=64, n_heads=4, mlp_dim=256, n_layers=6, n_classes=4, in_channels=3):
        super().__init__()
        self.patch_size = patch_size
        self.patch_embed = PatchEmbedding(img_size, patch_size, in_channels, emb_dim)
        n_patches = (img_size // patch_size) ** 2
        self.cls_token = nn.Parameter(torch.zeros(1, 1, emb_dim))
        self.pos_embed = nn.Parameter(torch.zeros(1, 1 + n_patches, emb_dim))  # n_patches + 1 cls
    ...
    def forward(self, x):
        B = x.shape[0]
        x = self.patch_embed(x)  # B x n_patches x emb_dim
        cls_token = self.cls_token.expand(B, -1, -1)  # B x 1 x emb_dim
        x = torch.cat([cls_token, x], dim=1)  # B x (1+n_patches) x emb_dim
